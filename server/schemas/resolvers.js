...
const resolvers = {
  ...
  Mutation: {
    ...
    saveRecipe: async (parent, { mealData }, context) => {
      ...
      // Check for validity of idMeal
      if (!mealData.idMeal || mealData.idMeal.trim() === "") {
        throw new Error("idMeal cannot be null, undefined, or an empty string.");
      }
    
      if (context.user) {
        const existingUser = await User.findOne({ _id: context.user._id });
        
        // Check if the recipe with the same idMeal already exists in the user's saved recipes
        if (existingUser.savedRecipes.some(recipe => recipe.idMeal === mealData.idMeal)) {
